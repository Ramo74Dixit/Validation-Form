...
export default function App() {
  ...
  const handleSubmit = (e) => {
    ...
    if (isValid) {
      ...
      window.alert("You can check your form entry at console");
    }
  };
  ...
  return (
    <div className="App min-h-screen flex items-center justify-center bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 p-6" style={{border:"2px solid black"}}>
      <div className="bg-slate-800 rounded-lg shadow-lg p-8 max-w-4xl w-full">
        <h1 className="text-2xl font-bold text-center mb-8 text-yellow-200">
          Registration Form
        </h1>
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-6">
            {['firstName', 'lastName', 'email', 'username', 'phoneNumber', 'panNumber', 'aadharNumber', 'country', 'city'].map((field, index) => (
            ...
            <div className="sm:col-span-3">
              ...
              <div className="mt-2 relative">
                ...
                {validationMessages.password && (
                  <p className="mt-2 text-sm text-red-600">{validationMessages.password}</p>
                )}
              </div>
            </div>
            <div className="sm:col-span-6 flex justify-center mt-6">
              <button
                type="submit"
                style={{margin:"4px",backgroundColor:"blue",color:"white",height:"30px"}}
